<script lang="ts">
    ...
    let { country, onUpdate }: { country: Country; onUpdate?: () => void } =
        $props();
    ...
    let country_name = $state<string>((country.country_name ?? "").trim());
    ...
    async function handleEditSubmit(e: Event) {
        ...
        let result = await invoke<boolean>("update_country", { new_country });
        showEditModal = false;
        if (onUpdate) onUpdate();
    }

    async function handleDelete() {
        let result = await invoke<boolean>("delete_country", {
...
<tr>
    <td class="px-4 py-2 border">{country.country_id}</td>
    <td class="px-4 py-2 border">{country.country_name}</td>
    <td class="px-4 py-2 border">
        ...
        {#if showEditModal}
            ...
            >
                <div class="bg-white p-6 rounded shadow-lg min-w-[300px]">
                    <h3 class="text-lg font-bold mb-4">Editar país</h3>
                    <form
        ...
        {#if showDeleteModal}
            ...
            >
                <div class="bg-white p-6 rounded shadow-lg min-w-[300px]">
                    <h3 class="text-lg font-bold mb-4">¿Eliminar país?</h3>
