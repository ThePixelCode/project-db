<script lang="ts">
    ...
    import CountryComponent from "./country.svelte";
    ...
    let country_name = $state<string>("");

    function process(page: number) {
        invoke<Country[]>("get_countries", { page }).then((value) => {
            countries = value;
        });
    }
    ...
    async function handleCreateSubmit(e: Event) {
        ...
        let result = await invoke<boolean>("create_country", { new_country });
        if (result) {
...
<table class="min-w-full border border-gray-300 mt-4">
    ...
    <tbody>
        {#each countries as country}
            <CountryComponent {country} onUpdate={() => process(page)} />
        {/each}
    </tbody>
</table>
