<script lang="ts">
    ...
    let author_id = $state<string>("");
    let country_id = $state<string>("");
    let author_name = $state<string>("");

    function process(page: number) {
        invoke<Author[]>("get_authors", { page }).then((value) => {
            authors = value;
        });
    }
    ...
    async function handleCreateSubmit(e: Event) {
        ...
        let result = await invoke<boolean>("create_author", { new_author });
        if (result) {
...
<table class="min-w-full border border-gray-300 mt-4">
    ...
    <tbody>
        {#each authors as author}
            <AuthorComponent {author} onUpdate={() => process(page)} />
        {/each}
    </tbody>
</table>
